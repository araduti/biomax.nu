...
export function GscEmptyState({
  ...
  const inner = (
    <>
      ...
      <ol className="space-y-2 font-sans text-small text-ink-body list-decimal pl-5">
        <li>
        ...
        <li>
          Sätt <code className="px-1 rounded bg-surface-warm font-mono text-caption">GSC_SERVICE_ACCOUNT_KEY</code>{" "}
          (hela JSON-filens innehåll) och{" "}
          <code className="px-1 rounded bg-surface-warm font-mono text-caption">GSC_PROPERTY</code>{" "}
          (t.ex. <code className="font-mono text-caption">https://www.biomax.nu/</code>) i
          produktionsmiljöns env.
        </li>
        <li>Starta om appen — datat dyker upp inom någon minut.</li>
      </ol>
      <p className="mt-4 font-sans text-caption text-ink-soft italic">
  ...
  if (variant === "inline") {
    return (
      <div className="rounded-xl border border-dashed border-border bg-surface-warm/40 px-4 py-3 font-sans text-small text-ink-mute">
