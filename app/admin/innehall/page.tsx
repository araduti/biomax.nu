    ...
    <>
      ...
      {totalBacklog === 0 && (
        <div className="rounded-xl border border-accent/30 bg-accent/[0.06] px-6 py-5">
          <p className="font-sans text-body-lg font-semibold text-primary-deep mb-1">
            ...
            {missingSources.map((m) => (
              ...
              >
                <Link
                  ...
                  target="_blank"
                  className="font-sans text-small text-primary-deep hover:text-primary truncate"
                >
                  {m.name}
                </Link>
          ...
          <ul className="divide-y divide-border-soft -mx-1">
            {unmatched.map((u) => (
              <li key={u.rowName} className="px-1 py-2.5">
                <p className="font-sans text-small font-medium text-primary-deep truncate">
                  {u.rowName}
                </p>
                <p className="font-sans text-caption text-ink-mute mt-0.5 truncate">
                  i {u.products.length}{" "}
            ...
            {thinFaq.map((p) => (
              <li key={p.slug} className="px-1 py-2.5">
                <div className="flex items-baseline justify-between gap-3">
                  ...
                  <span className="font-sans text-micro text-status-error tabular-nums whitespace-nowrap">
                    {p.faqCount}/2 frågor
                  </span>
                </div>
