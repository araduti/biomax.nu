...
single-page snapshot of every layer of the Korg stack. Each row
...
One page that answers "what is the whole Korg stack?" without
...
| Layer | Choice | Owning ADR / status |
|---|---|---|
| Database | Postgres (17 in Korg dev / 18 in Watchtower — version pin alignment is open) | ADR 0029 D4 |
...
| Search | Postgres FTS + `pgvector`; abstracted behind `@korg/search` | this ADR |
...
| Tenant resolution | Host parsed at Next 16 Proxy (`proxy.ts`), forwarded as `x-korg-tenant`, resolved via `currentTenant()` React.cache | ADR 0028 D3 |
...
| Authority planes | Two Better Auth instances, cookies `__korgsf` vs `__korgadm`, host-scoped, platform 2FA mandatory | ADR 0031 / ADR 0003 |
...
| UI primitives | `@korg/ui` (shadcn/radix-level) | ADR 0007 |
...
apps/
  ...
  console/                 Korg-operator-only (admin.korg.nu)
...
**First extraction:** `@korg/db` (today's `lib/prisma.ts` + `lib/tenant/*`).
**Hard prerequisite to `@korg/auth-storefront`:** un-`biomax`-ify
...
| Layer | Choice | Status |
|---|---|---|
| Auth (tenant plane) | Better Auth, cookie `__korgsf`, per-tenant domain | ADR 0003 (in prod, needs un-`biomax`-ify) |
| Auth (platform plane) | Better Auth, cookie `__korgadm`, host-scoped to admin.korg.nu, 2FA mandatory | ADR 0031 |
| Identity (Swedish, future) | **BankID** primary + **Freja eID** fallback — login, KYC, age verification, terms signing | year-1 must (no ADR) |
| Payments (default, future) | **Stripe Connect (Standard)** + Stripe Tax + Stripe Radar — Korg as platform, merchants as connected accounts | partnership target (no ADR) |
...
| Package | `@korg/ai` — prompt registry, mandatory prompt caching, per-tenant cost metering, agent runtime |
...
| Product surfaces | (1) Onboarding-as-conversation, (2) merchant in-admin assistant ("Korg Assist"), (3) optional per-tenant shopper-side assistant (AI-Pro tier) |
...
| Audit log | `@korg/audit` → dedicated table; D10 lockin features depend on it | this ADR |
...
| Secrets at runtime | `/etc/korg/app.env` on VM, mode 0600, hydrated by `sops -d` | this ADR |
...
| Skatteverket OSS quarterly report | future | year-1 must (Korg Bokslut, §11 D10) |
...
## 10. Per-tenant rehearsal — "Korg Repetition" (proposed — `D-repetition` ADR pending)
...
| Shareable preview | `korg.nu/r/abc123`, password-gated, watermarked, `noindex`, auto-expire |
...
|---|---|---|
| Korg Historik | Field-level change log + revert on every editable thing | Audit trail accountants rely on; foundation for the others |
| Korg Tidslinje | Unified per-customer event timeline | After 18 months, irreplaceable customer-context |
| Korg Lagerbok | Audit-grade inventory ledger | Bokföringslagen-required; accountants become Korg sales reps |
| Korg Bokslut | Quarterly + annual close: moms / OSS / FTI / Bokföringslagen-aligned | Highest-leverage lockin; replaces quarter-long spreadsheet hell |
| Korg Snapshot | Merchant-grade PITR over WAL-G | Once experienced, switching feels unsafe |
| Korg Inkorgen | Support inbox + AI templates trained on tenant's voice | Two years of saved replies = a junior support hire's worth of IP |
| Korg Pulsen | Multi-year cohort + LTV + per-product margin analytics | Worthless until ~18mo of data — then irreplaceable |
...
|---|---|---|---|
| Korg Start | 0 SEK | 1.4% + 1.50 SEK / Korg-payments txn | Subdomain only, full features, AI Assist Basic |
| Korg Växa | 199 SEK | 0.4% + 1.50 SEK | Custom domain, AI Assist Pro, advanced SEO |
| Korg Plus | 799 SEK | 0% (cost only) | B2B, subscriptions, POS, headless API, dedicated CSM, Korg Repetition (unlimited + AI synthetic shoppers) |
...
| Shopify Hydrogen / Medusa / Saleor | Korg builds the engine, doesn't consume one |
...
|---|---|---|
| **D-monorepo** | §5 layout, pnpm workspaces, package extraction sequence | Touches every file; rename `biomax.nu` → `korg` |
...
| **D-seo** | `@korg/seo` package, AIO/GEO, programmatic SEO | Quality gates for programmatic pages |
...
- Postgres version pin — 17 (Korg dev) vs 18 (Watchtower) — one
...
- ADR 0029 PeeringDB analysis (2026-05-18) — overrides any
  ...
  and corrected in §1.
- `docs/infra/korg-hosting-plan.md`, `docs/infra/korg-infra-shapes.md`
