...
**Date:** 2026-05-21
**Status:** **SUPERSEDED** by kine ADR 0001 (commit `2da9918` on
`github.com/araduti/kine`, `docs/adr/0001-consolidated-bill-of-materials.md`).

> ⚠️ **Frozen — do not edit.** This document was the meta-ADR while
> the platform lived in `biomax.nu`. On 2026-05-21 the kine repo was
> bootstrapped (commit `c926823`) and this BoM was copied across as
> kine ADR 0001. An audit then surfaced 10 categories of drift
> (broken cross-refs, stale forcing-function markers, "in prod" lies
> in kine context, biomax-specific paths surfaced as platform truth,
> internal Watchtower references, etc.). The kine copy was rewritten
> as a **Living** document with an explicit status taxonomy and a
> maintenance contract. This biomax file is now historical only.
>
> All future updates to the consolidated BoM happen in **kine ADR
> 0001**. Do not edit this file. Do not cite it from new ADRs.
> Cross-references in biomax ADRs 0001–0035 to "ADR 0036" remain
> valid as historical lineage; they are not broken just because
> this is superseded.
...
**Original status:** **Accepted** — meta-ADR. The tables below are the
